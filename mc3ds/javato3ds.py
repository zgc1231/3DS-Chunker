...
def iter_java_chunks(region_path: Path):
    region_pattern = re.compile(r"r\.(-?\d+)\.(-?\d+)\.mca")
    for region_file in region_path.glob("r.*.*.mca"):
...
def convert_java(java_world: Path, world_out: Path, delete_out: bool) -> None:
    ...
    if (java_world / "region").is_dir():
        region_path = java_world / "region"
    elif java_world.is_dir() and any(java_world.glob("r.*.*.mca")):
        region_path = java_world
    else:
        raise FileNotFoundError(
            "Java world path must be a world folder or the 'region' folder containing .mca files"
        )
    ...
    for chunk, chunk_x, chunk_z, dimension in iter_java_chunks(region_path):
